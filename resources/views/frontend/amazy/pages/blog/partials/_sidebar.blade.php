    ...
    <div class="blog_sidebar_wrap mb_30">
        <form action="{{url('/blog')}}" name="sidebar_search">
            <div class="input-group  theme_search_field4 w-100 mb_20 style2">
                ...
                <input type="text" class="form-control search_input" id="inlineFormInputGroup" placeholder="{{ __('blog.search_post') }}" value="{{request()->get('query')}}" name="query" required>
            </div>
        </form>
            ...
            <div class="news_lists">
                @foreach($popularPost as $post)
                    <div class="single_newslist">
                        ...
                        <p>{{dateConvert($post->published_at)}}</p>
                    </div>
                @endforeach
            </div>
        </div>
