...
class FilterRepository
{
    ...
    public function search_query($slug){
        $slugs = explode(' ',$slug);
            $mainProducts = Product::whereHas('tags', function($q) use($slug){
                return $q->where(function($q) use ($slug){
                    $q = $q->orWhere('name', 'LIKE', "%{$slug}%");
                    return $q;
                });
            })->pluck('id')->toArray();

        $products = SellerProduct::with('product')->activeSeller()->select('seller_products.*')
                                                  ->join('products', function($qq){
                                                  ...
                                                  ->whereHas('product', function($query) use($mainProducts,$slug){
                                                         return $query->whereIn('products.id',$mainProducts)
