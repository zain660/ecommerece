        ...
        @if(isModuleActive('FrontendMultiLang'))
                ...
                <ul class="nav nav-tabs justify-content-start mt-sm-md-20 mb-30 grid_gap_5" role="tablist">
                    @foreach ($LanguageList as $key => $language)
                        <li class="nav-item lang_code" data-id="{{$language->code}}">
                            <a class="nav-link anchore_color @if (auth()->user()->lang_code == $language->code) active @endif" href="#{{$working_process_modal_tab_id}}{{$language->code}}" role="tab" data-toggle="tab" aria-selected="@if (auth()->user()->lang_code == $language->code) true @else false @endif">{{ $language->native }} </a>
                        </li>
                    @endforeach
                </ul>
                <div class="tab-content">
                    @foreach ($LanguageList as $key => $language)
                        <div role="tabpanel" class="tab-pane fade @if (auth()->user()->lang_code == $language->code) show active @endif" id="{{$working_process_modal_tab_id}}{{$language->code}}">
                            <div class="row">                                
                                ...
                                <div class="col-xl-12">
                                    <div class="primary_input mb-35">
                                        ...
                                        <textarea name="description[{{$language->code}}]" placeholder="{{ __('common.description') }}" class="workProcessText" id="description{{$language->code}}">{{old('description.'.$language->code)}}</textarea>
                                    </div>
         ...
         @else
            ...
            <div class="col-xl-12">
                <div class="primary_input mb-35">
                    <label class="primary_input_label" for="">{{ __('common.details') }} <span class="text-danger">*</span></label>
                    <textarea name="description" placeholder="{{ __('common.description') }}" class="workProcessText" id="description"></textarea>
                </div>
