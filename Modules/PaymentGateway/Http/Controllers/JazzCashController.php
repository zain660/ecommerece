    ...
    public function paymentStatus(Request $request)
	{
		...
		if($response['pp_ResponseCode'] == '000')
		{
            if ($response['pp_BillReference'] == "walletRecharge") {
                ...
                return $walletService->walletRecharge($amount, $credential->method->id, $response['pp_TxnRefNo']);
            }
            if ($response['pp_BillReference'] == "checkoutPay") {
                $amount = $response['pp_Amount'] / 100;
                $orderPaymentService = new OrderRepository;
                $order_payment = $orderPaymentService->orderPaymentDone($amount, $credential->method->id, $response['pp_TxnRefNo'], (auth()->check())?auth()->user():null);
