...
class CouponController extends Controller
{
    ...
    public function getData(){
        ...
        return DataTables::of($coupon)
            ...
            ->addColumn('start_date', function($coupon){
                return dateConvert($coupon->start_date);
            })
            ->addColumn('end_date', function($coupon){
                return dateConvert($coupon->end_date);
            })
    ...
    public function getForm(Request $request){
        try{
            $id = $request->id;
            if($id == 1){
                return view('marketing::coupon.components.form_data_for_product');
            }elseif($id == 2){
    ...
    public function edit(Request $request){
        try{
            $coupon = $this->couponService->editById($request->id);
            return view('marketing::coupon.components.edit',compact('coupon'));
        }catch(Exception $e){
            LogActivity::errorLog($e->getMessage());
        }
    }
