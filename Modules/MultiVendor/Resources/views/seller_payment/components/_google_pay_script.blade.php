...
@push('scripts')
    ...
    <script type="text/javascript">
        ...
        */
        ...
        {
            $.post('{{ route('googlePay.payment_status') }}', {_token:'{{ csrf_token() }}', purpose:'SubscriptionPayment', amount:'{{ $recharge_amount }}', requestId:el}, function(data){
               if(data == 1){
                   toastr.success("{{__('common.transaction_successfully')}}","{{__('common.success')}}")
                   location.replace("{{ route('seller.dashboard') }}");
               }
               else{
