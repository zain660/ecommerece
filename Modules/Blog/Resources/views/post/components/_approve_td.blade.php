@if (is_admin_user(auth()->user()->role_id))
    <label class="switch_toggle" for="active_checkbox{{ $value->id }}">
        <input type="checkbox" id="active_checkbox{{ $value->id }}" @if ($value->is_approved == 1) checked {{ $value->is_approved ? 'checked' : '' }}@endif class="approved" data-value="{{ $value->id }}" value="{{ $value->id }}">
